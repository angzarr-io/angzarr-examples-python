"""Hand bounded-context service entrypoint.
...
(``HandAggregate``) before returning a ``BusinessResponse``. There is one GIL,
...
def build_router() -> _az.Router:
    """An FFI router with the Hand aggregate registered. Caller owns close()."""
    router = _az.Router()
