"""Tournament bounded-context service entrypoint.
...
(``TournamentAggregate``) before returning a ``BusinessResponse``. There is one
...
def build_router() -> _az.Router:
    """An FFI router with the Tournament aggregate registered. Caller owns close()."""
    router = _az.Router()
    register_tournament_aggregate(router, TournamentAggregate())
    return router
