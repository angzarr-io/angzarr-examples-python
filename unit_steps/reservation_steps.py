...
@given('a pending tournament registration "{res}" for tournament "{trn}" with fee {fee:d}')
...
@given('a pending rebuy "{res}" for tournament "{trn}" at table "{table}" seat {seat:d} with fee {fee:d}')
...
def _when_initiate_buy_in(context, who, seat, table, n):
    context.world.dispatch(
        DOMAIN,
        P + "InitiateBuyIn",
        buy_in.InitiateBuyIn(player_root=uuid_for(who), table_root=uuid_for(table), seat=seat, amount=_chips(n)),
    )


@when('{who} initiates a buy-in for seat {seat:d} at an empty table name for {n:d} chips')
def _when_initiate_buy_in_no_table(context, who, seat, n):
    context.world.dispatch(
        DOMAIN,
        P + "InitiateBuyIn",
        buy_in.InitiateBuyIn(player_root=uuid_for(who), table_root=b"", seat=seat, amount=_chips(n)),
    )


@when('{who}\'s buy-in "{res}" is confirmed')
def _when_confirm_buy_in(context, who, res):
    context.world.dispatch(DOMAIN, P + "ConfirmBuyIn", buy_in.ConfirmBuyIn(reservation_id=uuid_for(res)))


@when('{who}\'s buy-in with an empty reservation identifier is confirmed')
def _when_confirm_buy_in_empty(context, who):
    context.world.dispatch(DOMAIN, P + "ConfirmBuyIn", buy_in.ConfirmBuyIn(reservation_id=b""))


@when('{who}\'s buy-in "{res}" is released because of "{reason}"')
def _when_release_buy_in(context, who, res, reason):
    context.world.dispatch(DOMAIN, P + "ReleaseBuyIn", buy_in.ReleaseBuyIn(reservation_id=uuid_for(res), reason=reason))


@when('{who}\'s buy-in with an empty reservation identifier is released because of "{reason}"')
def _when_release_buy_in_empty(context, who, reason):
    context.world.dispatch(DOMAIN, P + "ReleaseBuyIn", buy_in.ReleaseBuyIn(reservation_id=b"", reason=reason))
...
def _when_initiate_registration(context, who, trn):
    context.world.dispatch(
        ...
        registration.InitiateTournamentRegistration(player_root=uuid_for(who), tournament_root=uuid_for(trn)),
    )
...
def _when_initiate_registration_no_trn(context, who):
    context.world.dispatch(
        DOMAIN,
        P + "InitiateTournamentRegistration",
        registration.InitiateTournamentRegistration(player_root=uuid_for(who), tournament_root=b""),
    )
...
def _when_confirm_registration(context, who, res):
    context.world.dispatch(
        DOMAIN, P + "ConfirmRegistrationFee", registration.ConfirmRegistrationFee(reservation_id=uuid_for(res))
    )


@when('{who}\'s tournament registration with an empty reservation identifier is confirmed')
def _when_confirm_registration_empty(context, who):
    context.world.dispatch(
        DOMAIN, P + "ConfirmRegistrationFee", registration.ConfirmRegistrationFee(reservation_id=b"")
    )
...
def _when_release_registration(context, who, res, reason):
    context.world.dispatch(
        ...
        registration.ReleaseRegistrationFee(reservation_id=uuid_for(res), reason=reason),
    )


@when('{who}\'s tournament registration with an empty reservation identifier is released because of "{reason}"')
def _when_release_registration_empty(context, who, reason):
    context.world.dispatch(
        DOMAIN, P + "ReleaseRegistrationFee", registration.ReleaseRegistrationFee(reservation_id=b"", reason=reason)
    )
...
def _when_initiate_rebuy(context, who, trn, table, seat):
    context.world.dispatch(
        ...
        P + "InitiateRebuy",
        rebuy.InitiateRebuy(
            player_root=uuid_for(who), tournament_root=uuid_for(trn), table_root=uuid_for(table), seat=seat
        ),
    )


@when('{who} initiates a rebuy for an empty tournament name at table "{table}" seat {seat:d}')
def _when_initiate_rebuy_no_trn(context, who, table, seat):
    context.world.dispatch(
        DOMAIN,
        P + "InitiateRebuy",
        rebuy.InitiateRebuy(player_root=uuid_for(who), tournament_root=b"", table_root=uuid_for(table), seat=seat),
    )


@when('{who} initiates a rebuy for tournament "{trn}" at an empty table name seat {seat:d}')
def _when_initiate_rebuy_no_table(context, who, trn, seat):
    context.world.dispatch(
        DOMAIN,
        P + "InitiateRebuy",
        rebuy.InitiateRebuy(player_root=uuid_for(who), tournament_root=uuid_for(trn), table_root=b"", seat=seat),
    )


@when('{who}\'s rebuy "{res}" is confirmed')
def _when_confirm_rebuy(context, who, res):
    context.world.dispatch(DOMAIN, P + "ConfirmRebuyFee", rebuy.ConfirmRebuyFee(reservation_id=uuid_for(res)))


@when('{who}\'s rebuy with an empty reservation identifier is confirmed')
def _when_confirm_rebuy_empty(context, who):
    context.world.dispatch(DOMAIN, P + "ConfirmRebuyFee", rebuy.ConfirmRebuyFee(reservation_id=b""))


@when('{who}\'s rebuy "{res}" is released because of "{reason}"')
def _when_release_rebuy(context, who, res, reason):
    context.world.dispatch(DOMAIN, P + "ReleaseRebuyFee", rebuy.ReleaseRebuyFee(reservation_id=uuid_for(res), reason=reason))


@when('{who}\'s rebuy with an empty reservation identifier is released because of "{reason}"')
def _when_release_rebuy_empty(context, who, reason):
    context.world.dispatch(DOMAIN, P + "ReleaseRebuyFee", rebuy.ReleaseRebuyFee(reservation_id=b"", reason=reason))


# --- Then: records (initiate) ---


@then('a pending buy-in is recorded for seat {seat:d} at table "{table}" for {n:d} chips')
@then('a pending buy-in is recorded for seat {seat:d} at table "{table}" for {n:d} chip')
...
def _then_registration_recorded(context, trn):
    ev = context.world.emitted(P + "RegistrationRequested", registration.RegistrationRequested())
    assert ev.tournament_root == uuid_for(trn), "tournament mismatch"


@then('a pending rebuy is recorded for tournament "{trn}" at table "{table}" seat {seat:d}')
...
@then('the buy-in "{res}" is confirmed for seat {seat:d} at table "{table}" for {n:d} chips')
...
def _then_buy_in_released(context, res, reason):
    ev = context.world.emitted(P + "BuyInReservationReleased", buy_in.BuyInReservationReleased())
    assert ev.reservation_id == uuid_for(res) and ev.reason == reason, "reservation/reason mismatch"


@then('the tournament registration "{res}" is confirmed for tournament "{trn}" with fee {fee:d}')
def _then_registration_confirmed(context, res, trn, fee):
    ev = context.world.emitted(P + "RegistrationFeeConfirmed", registration.RegistrationFeeConfirmed())
    assert ev.reservation_id == uuid_for(res) and ev.tournament_root == uuid_for(trn), "reservation/tournament mismatch"
...
def _then_registration_released(context, res, reason):
    ev = context.world.emitted(P + "RegistrationFeeReleased", registration.RegistrationFeeReleased())
    assert ev.reservation_id == uuid_for(res) and ev.reason == reason, "reservation/reason mismatch"
