...
def make_event_book(
    ...
    return types.EventBook(
        cover=types.Cover(
            domain=domain,
            root=types.BytesValue(value=root),
...
def make_notification(
    ...
    rejected_cmd = make_command_book(rejected_domain, rejected_command_type)
...
def step_player_with_state(context):
    ...
    event_book = make_event_book("player")
    context.aggregate = TestPlayerWithRejectionHandler(event_book)
    # Apply state from table
    for row in context.table:
        if row["reserved_amount"]:
            context.aggregate._state = PlayerState(
...
def step_rejected_handler_for(context, domain, command):
    """Verify aggregate has rejection handler."""
    key = f"{domain}/{command}"
    assert key in context.aggregate._rejection_table, f"No rejection handler for {key}"
...
def step_aggregate_receives_notification(context):
    """Aggregate receives Notification."""
    for row in context.table:
        domain = row["domain"]
        command = row["command"]
...
def step_notification_arrives(context):
    ...
    for row in context.table:
        domain = row["rejected_domain"]
        command = row["rejected_command"]
...
def step_handler_receives_notification(context):
    ...
    assert hasattr(context.aggregate, "_rejection_context")
    assert context.aggregate._rejection_context == context.notification
...
def step_saga_command_with(context):
    ...
    for row in context.table:
        if "domain" in row:
            context.rejected_domain = row["domain"]
        if "command_type" in row:
            context.rejected_command = row["command_type"]
